...
import voice_a from '../voice_a'
import voice_b from '../voice_b'
import voice_c from '../voice_c'
import voice_d from '../voice_d'
...
const scale = [0,2,4,5,7,9,11,12,14,16,17,19,21,23,24,26,28,29,31,33,35,36]
...
function quartet(){
    ...
    if (change===0) {
        ...
        voice_a.note(adjustedPitch, allVolumes[count]);
        
        voice_b.note(TadjustedPitch, TallVolumes[count])
    ...
    if (Mchange===0) {
        ...
        voice_c.note(MadjustedPitch, MallVolumes[Mcount])

        var Wpitch = WallNotes[Mcount]
        var WadjustedPitch = scale[Wpitch+shift]+Mregister+modulate
        voice_d.note(WadjustedPitch, WallVolumes[Mcount])
    }
}
