...
const shuffle=(idx, numToShuffle)=>{
    const length = motifs[idx][0].length
    if (!numToShuffle) {
        numToShuffle = rand(1, length)
    }
    if (numToShuffle === 'all') {
        numToShuffle = length
    }
    ...
    valuesToMove.forEach(val => newArr.splice(rand(0, newArr.length), 0, val))
    motifs[idx].push(newArr)
}
...
const makePhrase=()=>{
    const makeProgression=()=>{
        ...
        let maxShifts = 8
        let minShifts = 4
        const numShifts = rand(minShifts, maxShifts)
        const progression = []
...
const lazyComposer=()=>{
 ...
 //it needs to compile phrases from the original and variations